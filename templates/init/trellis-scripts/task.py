...
# =============================================================================
# Command: create-from-phase (CCG custom)
# =============================================================================

def cmd_create_from_phase(args: argparse.Namespace) -> int:
    """Create task from OpenSpec change phase(s)."""
    repo_root = get_repo_root()
    change_dir = args.change
    dev_type = args.dev_type or "fullstack"
    priority = args.priority or "P2"

    # Parse phase(s)
    if args.phase and args.phases:
        print(colored("Error: --phase and --phases are mutually exclusive", Colors.RED), file=sys.stderr)
        return 1

    if not args.phase and not args.phases:
        print(colored("Error: --phase or --phases is required", Colors.RED), file=sys.stderr)
        return 1
    ...
    if args.phases:
        ...
        phase_list = [int(args.phase)]

    is_multi = len(phase_list) > 1

    # Validate change directory and tasks.md
    change_path = repo_root / change_dir
    if not change_path.is_dir():
        print(colored(f"Error: Change directory not found: {change_dir}", Colors.RED), file=sys.stderr)
        return 1

    tasks_md = change_path / "tasks.md"
    if not tasks_md.is_file():
        print(colored(f"Error: tasks.md not found in {change_dir}", Colors.RED), file=sys.stderr)
        return 1

    tasks_content = tasks_md.read_text(encoding="utf-8")

    # Extract phase titles
    phase_titles: list[str] = []
    for pn in phase_list:
        match = re.search(rf"^## Phase {pn}:\s*(.+)$", tasks_content, re.MULTILINE)
        if not match:
            print(colored(f"Error: Phase {pn} not found in tasks.md", Colors.RED), file=sys.stderr)
            return 1
        phase_titles.append(match.group(1).strip())

    # Generate slug and title
    change_name = Path(change_dir).name
    first_phase = phase_list[0]

    if is_multi:
        phase_suffix = "-".join(str(p) for p in phase_list)
        slug = f"{change_name}-phase-{phase_suffix}"
        phase_csv = ",".join(str(p) for p in phase_list)
        titles_joined = " + ".join(phase_titles)
        display_title = f"Phase {phase_csv}: {titles_joined}"
    else:
        slug = f"{change_name}-phase-{first_phase}"
        display_title = f"Phase {first_phase}: {phase_titles[0]}"
    ...
    # Build task.json
    phase_desc = (
        f"Execute Phases {','.join(str(p) for p in phase_list)} of OpenSpec change: {change_name}"
        if is_multi
        else f"Execute Phase {first_phase} of OpenSpec change: {change_name}"
    )
    ...
    task_data: dict = {
        ...
        "subtasks": [],
        "relatedFiles": [],
        "notes": "Auto-generated from OpenSpec change" + (" (merged phases)" if is_multi else ""),
    }
    if is_multi:
        task_data["phase_numbers"] = phase_list

    _write_json_file(task_dir / FILE_TASK_JSON, task_data)

    # Create prd.md
    _write_create_from_phase_prd(task_dir, change_dir, phase_list, phase_titles, is_multi)

    # Create jsonl files
    _write_create_from_phase_jsonl(task_dir, change_dir, dev_type, phase_list, is_multi, repo_root)

    # Output summary
    print(colored("Created task from OpenSpec phase:", Colors.GREEN), file=sys.stderr)
    print(f"  Task: {dir_name}", file=sys.stderr)
    print(f"  Change: {change_dir}", file=sys.stderr)
    if is_multi:
        for i, pn in enumerate(phase_list):
            print(f"    - Phase {pn}: {phase_titles[i]}", file=sys.stderr)
    else:
        print(f"  Phase: {first_phase} - {phase_titles[0]}", file=sys.stderr)
    print(f"  Dev Type: {dev_type}", file=sys.stderr)
    print(file=sys.stderr)
...
def _write_create_from_phase_prd(
    task_dir: Path, change_dir: str, phase_list: list[int],
    phase_titles: list[str], is_multi: bool,
) -> None:
    """Write prd.md for create-from-phase task."""
    lines: list[str] = []
    if is_multi:
        phase_csv = ",".join(str(p) for p in phase_list)
        lines.append("# Task: 执行 OpenSpec Change 多 Phase 合并任务\n")
        lines.append(f"## Change\n{change_dir}\n")
        lines.append("## Phases（必须全部执行）")
        for i, pn in enumerate(phase_list):
            lines.append(f"- Phase {pn}: {phase_titles[i]}")
        lines.append(f"\n## 说明\n本 Task 合并了 Phase {phase_csv}（规范上下文相同），必须按顺序全部执行。\n")
        lines.append("按照 ccg-impl 工作流：")
        for pn in phase_list:
            lines.append(f"- 读取 {change_dir}/tasks.md 中 Phase {pn} 的任务列表并实现")
        lines.append("\n每个 Phase 的执行步骤：")
        lines.append(f"1. 读取 {change_dir}/specs.md 和 design.md 理解规范和设计")
        lines.append("2. 多模型协作实现（后端用 Codex，前端用 Gemini）")
        lines.append("3. 外部模型只返回 diff patch，Claude 重写为生产代码")
        lines.append("4. 完成后更新 tasks.md 标记 [x]")
    else:
        first = phase_list[0]
        lines.append("# Task: 执行 OpenSpec Change Phase\n")
        lines.append(f"## Change\n{change_dir}\n")
        lines.append(f"## Phase\n{first}\n")
        lines.append(f"## Phase 标题\n{phase_titles[0]}\n")
        lines.append(f"## 说明\n执行上述 change 的 Phase {first} 任务。\n")
        lines.append("按照 ccg-impl 工作流：")
        lines.append(f"1. 读取 {change_dir}/tasks.md 中 Phase {first} 的任务列表")
        lines.append(f"2. 读取 {change_dir}/specs.md 和 design.md 理解规范和设计")
        lines.append("3. 多模型协作实现（后端用 Codex，前端用 Gemini）")
        lines.append("4. 外部模型只返回 diff patch，Claude 重写为生产代码")
        lines.append("5. 完成后更新 tasks.md 标记 [x]")

    (task_dir / "prd.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def _write_create_from_phase_jsonl(
    task_dir: Path, change_dir: str, dev_type: str,
    phase_list: list[int], is_multi: bool, repo_root: Path,
) -> None:
    """Write implement/check/debug.jsonl for create-from-phase task."""
    phase_csv = ",".join(str(p) for p in phase_list)
    tasks_reason = f"OpenSpec task list (Phase {phase_csv})" if is_multi else "OpenSpec task list"
    ...
    if dev_type in ("backend", "test"):
        ...
        entries.extend(get_implement_backend())
        entries.extend(get_implement_frontend())
    entries.append({"file": f"{change_dir}/specs.md", "reason": "OpenSpec requirements and constraints"})
    entries.append({"file": f"{change_dir}/design.md", "reason": "OpenSpec technical design"})
    entries.append({"file": f"{change_dir}/tasks.md", "reason": tasks_reason})
    _write_jsonl(task_dir / "implement.jsonl", entries)

    # check.jsonl
    check_entries = get_check_context(dev_type, repo_root)
    check_entries.append({"file": f"{change_dir}/specs.md", "reason": "Verify against OpenSpec requirements"})
    check_entries.append({"file": f"{change_dir}/tasks.md", "reason": f"Verify task completion ({tasks_reason})"})
    _write_jsonl(task_dir / "check.jsonl", check_entries)

    # debug.jsonl
    debug_entries = get_debug_context(dev_type, repo_root)
    debug_entries.append({"file": f"{change_dir}/specs.md", "reason": "Debug against OpenSpec requirements"})
    debug_entries.append({"file": f"{change_dir}/tasks.md", "reason": f"Debug context ({tasks_reason})"})
    _write_jsonl(task_dir / "debug.jsonl", debug_entries)
...
Usage:
  ...
  python3 task.py create-from-phase --change <dir> --phase <N> [--dev-type <type>]
  python3 task.py create-from-phase --change <dir> --phases "1,2,4" [--dev-type <type>]
...
def main() -> int:
    ...
    # create-from-phase (CCG custom)
    p_cfp = subparsers.add_parser("create-from-phase", help="Create task from OpenSpec phase")
    p_cfp.add_argument("--change", "-c", required=True, help="OpenSpec change directory")
    p_cfp.add_argument("--phase", help="Single phase number")
    p_cfp.add_argument("--phases", help="Comma-separated phase numbers (e.g. 1,2,4)")
    p_cfp.add_argument("--dev-type", "-t", help="Dev type: backend|frontend|fullstack|test")
    p_cfp.add_argument("--priority", default="P2", help="Priority (P0-P3)")
    ...
    commands = {
        ...
        "create-from-phase": cmd_create_from_phase,
    }
