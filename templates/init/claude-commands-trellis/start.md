...
From Brainstorm (Complex Task):
  PRD confirmed → Research → Configure Context → Activate → Choose Mode → Implement → Check → Complete

From Simple Task:
  Confirm → Create Task → Write PRD → Research → Configure Context → Activate → Choose Mode → Implement → Check → Complete
```
...
**Step 8: Choose Execution Mode** `[AI]`

**MANDATORY**: 实现前必须询问用户选择执行模式：

> 任务已就绪，请选择执行模式：
>
> 1. **Agent 模式** - 委派给 subagent（ccg-impl/ccg-review）执行。质量更高，code-spec context 通过 hook 自动注入。
> 2. **直接模式** - 我在当前会话直接实现。
...
If yes, resume from the appropriate step (usually Step 8 or 9).
